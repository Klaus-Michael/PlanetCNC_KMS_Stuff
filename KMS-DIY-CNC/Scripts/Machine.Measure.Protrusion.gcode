...
(dlg,data::MeasureProtrusion, typ=image, x=0)
(dlg,|X+|X-|Y+|Y-, typ=checkbox, x=50, w=425, def=1, param=strt)
(dlg,Protrusion size, typ=label, x=20, w=455, color=0xffa500)
...
o<msr> if [#<axis> EQ 0]
  #<_measureX> = [[#<measure1> + #<measure2>] / 2]
  #<sizeX> = [ABS[#<measure1> - #<measure2>]]
  G53 G00 X#<_measureX>
o<msr> elseif [#<axis> EQ 1]
  #<_measureY> = [[#<measure1> + #<measure2>] / 2]
  #<sizeY> = [ABS[#<measure1> - #<measure2>]]
  G53 G00 Y#<_measureY>
o<msr> else
  (msg,Error)
  #<_motion_maxacc>=#<_orig_maxacc> ;restore original acceleration settings
...
G91 G53 G00 H#<axis> E[-#<dir> * [#<dist> + #<size>/2]]
G90
G53 G00 Z#<slow_z>
G53 G01 Z#<startz>  F#<_slow_z_hight_speed>
#<pos> = #<_machine_axis|#<axis>>
